...
def load_internal_data(cfg: ExperimentConfig):
    ...
    # Store all split-safe preprocessing objects required to reconstruct
    # the exact Experiment 1 input space in later explainability experiments.
    # This prevents Experiment 2 from re-inferring domains or scaling and
    # guarantees architectural consistency with the trained checkpoint.
    metadata = {
        ...
        "domain_indices": {d: [int(i) for i in idx] for d, idx in domain_indices.items()},
        ...
        "feature_preprocessing": {
            "dropped_identifier_policy": "drop SEQN/id/participant_id/index/unnamed: 0 and high-cardinality *_id columns",
            "categorical_encoders": encoders,
            "train_medians": {c: float(medians[c]) for c in feature_columns},
            "standard_scaler_mean": {c: float(v) for c, v in zip(feature_columns, scaler.mean_)},
            "standard_scaler_scale": {c: float(v) for c, v in zip(feature_columns, scaler.scale_)},
        },
        "architecture": {
            "model_class": "NashCooperativeOrganizationalModel",
            "hidden_dim": int(cfg.hidden_dim),
            "learners_per_team": int(cfg.learners_per_team),
            "dropout": float(cfg.dropout),
            "num_classes": int(len(target_mapping)),
            "team_names": list(domain_indices.keys()),
            "team_input_dims": {d: int(len(idx)) for d, idx in domain_indices.items()},
        },
    }
    ...
    for epoch in range(1, CFG.max_epochs + 1):
        ...
        if val_metrics["f1_macro"] > best_val_f1:
            ...
            checkpoint_payload = {
                ...
                "domain_indices": {d: [int(i) for i in idx] for d, idx in domain_indices.items()},
                "domain_features": metadata["domain_features"],
                "feature_columns": metadata["feature_columns"],
                "target_column": metadata["target_column"],
                "target_mapping": metadata["target_mapping"],
                "feature_preprocessing": metadata["feature_preprocessing"],
                "architecture": metadata["architecture"],
                "best_epoch": int(best_epoch),
                "best_val_f1_macro": float(best_val_f1),
                "checkpoint_purpose": (
                    "Reusable Experiment 1 organizational checkpoint for subsequent "
                    "specialist-team explainability, ablation, and temporal validation experiments."
                ),
            }
            torch.save(checkpoint_payload, best_path)

            # Also save a JSON-readable checkpoint manifest so later scripts can
            # verify that they are using the exact architecture and preprocessing.
            manifest = {k: v for k, v in checkpoint_payload.items() if k != "model_state_dict"}
            with open(OUTPUT_DIR / "experiment1_checkpoint_manifest.json", "w", encoding="utf-8") as f:
